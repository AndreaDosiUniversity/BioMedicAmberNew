...
from unetr_pp.network_architecture.synapse.amber import AmberThreeDim
...
class amber_trainer_synapse(Trainer_synapse):
    """
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            assert isinstance(self.network, (AmberThreeDim, SegmentationNetwork, nn.DataParallel))
        else:
    ...
    def initialize_network(self):
        ...
        print("THE TYPE PF THE INPUT CHANNELS IS ", type(self.input_channels))
        print("THE NUMBER OF CLASSES ARE", self.num_classes)
        print("THE CROP SIZE IS", self.crop_size)
        print("THE EMBEDDING DIM IS", self.embedding_dim)
        print("THE DEPTHS ARE", self.depths)



        self.network = AmberThreeDim(in_chans=self.input_channels,
                             num_classes=self.num_classes,
                             img_size=tuple(self.crop_size),
                             #feature_size=16,
                             num_heads=[1, 2, 4, 8],
                             mlp_ratios=[4, 4, 4, 4],
                             qkv_bias=False,
