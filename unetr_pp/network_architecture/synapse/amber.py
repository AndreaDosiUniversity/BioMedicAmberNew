...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        #print("THE INITIAL X SHAPE IS: ", x.shape)
        # = x.permute(0, 4, 1, 2, 3) 
        x = self.proj(x)
        #print("THE X AFTER THE PROJ IS: ", x.shape)
        _, _, D, H, W = x.shape
...
class Attention(nn.Module):
    ...
    def forward(self, x, D, H, W ):
        ...
        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).contiguous().reshape(B, C, D, H, W).contiguous()
            x_ = self.sr(x_).reshape(B, C, -1).contiguous().permute(0, 2, 1).contiguous()
            x_ = self.norm(x_)
...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=1, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        if isinstance(img_size, int):
            ...
            self.patch_embed3 = OverlapPatchEmbed(img_size=img_size // 2, patch_size=2, stride=2, in_chans=embed_dims[1],
                                              embed_dim=embed_dims[2])
            self.patch_embed4 = OverlapPatchEmbed(img_size=img_size // 4, patch_size=2, stride=1, in_chans=embed_dims[2],
            ...
            self.patch_embed2 = OverlapPatchEmbed(img_size=(img_size[0] // 2, img_size[1] //2, img_size[2]//2), patch_size=3, stride=1, in_chans=embed_dims[0],
                                              embed_dim=embed_dims[1])
            self.patch_embed3 = OverlapPatchEmbed(img_size=(img_size[0] // 2, img_size[1]//2, img_size[2]//2), patch_size=2, stride=2, in_chans=embed_dims[1],
                                              embed_dim=embed_dims[2])
            self.patch_embed4 = OverlapPatchEmbed(img_size=(img_size[0] // 4, img_size[1]//4, img_size[2]//4), patch_size=2, stride=1, in_chans=embed_dims[2],
                                              embed_dim=embed_dims[3])
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.block1):
            x = blk(x, D, H, W)
        x = self.norm1(x)
        x = x.reshape(B, D, H, W, -1).contiguous().permute(0, 4, 1, 2, 3).contiguous()
        outs.append(x)
...
class SegFormerHead(nn.Module):
    ...
    """
    def __init__(self, feature_strides, img_size, in_channels=[64, 128, 320, 512] ,embedding_dim = 768, num_classes=19, dropout_ratio=0.1):
        super().__init__()
        ...
        #self.reduce_dim = 
        #self.proj = nn.Linear((self.image_size[0]//2+1)*(self.image_size[1]//2 +1)**2, (self.image_size[1]//2 +1)**2)
        self.dense_conv = nn.Sequential(
            nn.Conv3d(embedding_dim, self.num_classes, kernel_size = 1),
            nn.ReLU(),
            nn.BatchNorm3d(self.num_classes)
        )
        self.linear_pred = nn.Conv3d(self.num_classes, self.num_classes, kernel_size = 1 , bias=False)
        #self.linear_pred = nn.Conv3d(embedding_dim, self.num_classes, kernel_size = 1 , bias=False)
    ...
    def forward(self, x):
        ...
        n, _, d, h, w = c4.shape
        _c4 = self.linear_c4(c4).permute(0,2,1).contiguous().reshape(n, -1, c4.shape[2], c4.shape[3], c4.shape[4])
        _c4 = self.resize(_c4, size=c1.size()[2:],mode='trilinear',align_corners=False)

        _c3 = self.linear_c3(c3).permute(0,2,1).contiguous().reshape(n, -1, c3.shape[2], c3.shape[3], c3.shape[4])
        _c3 = self.resize(_c3, size=c1.size()[2:],mode='trilinear',align_corners=False)

        _c2 = self.linear_c2(c2).permute(0,2,1).contiguous().reshape(n, -1, c2.shape[2], c2.shape[3], c2.shape[4])
        _c2 = self.resize(_c2, size=c1.size()[2:],mode='trilinear',align_corners=False)

        _c1 = self.linear_c1(c1).permute(0,2,1).contiguous().reshape(n, -1, c1.shape[2], c1.shape[3], c1.shape[4])

        _c = self.linear_fuse(torch.cat([_c4, _c3, _c2, _c1], dim=1))

        x = self.dropout(_c)
        #print("THE SIZE BEFORE THE REDUCE DIM IS ", x.shape)
        #this last layer reduce the spectral dimension and perform a dense prediction at the same time
        #x = self.linear_pred(x)
        #x = torch.mean(x, dim=2)
        #x = self.resize(x, size=self.image_size, mode='trilinear',align_corners=False)
        x = self.dense_conv(x)
        x = self.linear_pred(x)
        return x
    

class AmberThreeDim(SegmentationNetwork):
    def __init__(self, img_size=(200,224,224), patch_size=4, in_chans=1, num_classes=1000, 
